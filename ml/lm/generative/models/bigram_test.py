...
from ml.lm.generative.models.bigram import Bigram


class TestBigramModel(unittest.TestCase):
    def setUp(self):
        self.cfg = ModelConfig(vocab_size=27)
        self.model = Bigram(self.cfg)

    def test_init(self):
        self.assertEqual(
            self.model.logits.shape, (self.cfg.vocab_size, self.cfg.vocab_size)
        )
        self.assertTrue(
            torch.equal(
                self.model.logits,
                torch.zeros(self.cfg.vocab_size, self.cfg.vocab_size),
            )
        )
        self.assertIn(
            self.model.logits,
    ...
    def test_forward_no_targets(self):
        idx = torch.randint(0, self.cfg.vocab_size, (5,))  # 5 random indices
    ...
    def test_forward_with_targets(self):
        idx = torch.randint(0, self.cfg.vocab_size, (5,))  # Random indices
        targets = torch.randint(0, self.cfg.vocab_size, (5,))  # Random targets
    ...
    def test_forward_equivalence(self):
        ...
        x = F.one_hot(idx, num_classes=self.cfg.vocab_size).float()
        self.assertTrue(
    ...
    def test_generate(self):
        ...
        self.assertTrue(
            ...
            and torch.all(generated_seq < self.cfg.vocab_size),
